...
The `navgateMany` Transform can utilize a [Locator](/start/locators) with the special key `"$link"` to identify all the link elements (i.e. `<a>` elements) on the page that should be followed.
